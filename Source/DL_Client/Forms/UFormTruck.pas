...
begin
  ...
  begin
    ...
    Edt_PreValue.Text := FieldByName('T_PrePValue').AsString;
...
begin
  ...
  nPrePValue:= StrToFloatDef(Trim(Edt_PreValue.Text), 0);
  if (CheckUserP.Checked)AND(nPrePValue<=0) then
  begin
    ShowMsg('请输入有效预置皮重重量', sHint);
    Exit;
  end;
  ...
  begin
    nStr := MakeSQLByStr([SF('T_Truck', nTruck),
            ...
            SF('T_PrePValue', FloatToStr(nPrePValue)),
            SF('T_PValue', FloatToStr(nPrePValue)),
