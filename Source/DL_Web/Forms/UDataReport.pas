...
type
  ...
  public
    { Public declarations }
    procedure OnReViewReport(const nRe:Boolean);
    procedure ShowReport(nPdfUrl:string);
...
uses ServerModule, UfrmViewReport;
...
procedure TFDR.OnReViewReport(const nRe:Boolean);
begin
    FPrintSuccess := nRe;
end;

//Desc: 预览报表
procedure TFDR.ShowReport(nPdfUrl:string);
begin
  FPrintSuccess := False;
  ShowViewReportForm(nPdfUrl, OnReViewReport);
end;
...
//Desc: 生成PDF报表
function TFDR.GenReportPDF: string;
begin
  FPrintSuccess := False;

  try
    PDFeXPrt1.Background := True;
    PDFeXPrt1.ShowProgress := False;
    PDFeXPrt1.ShowDialog := False;
    //PDFeXPrt1.FileName := '\files\'+FormatDateTime('yyyyMMddhhnnsszzz', Now)+'.pdf';//
    PDFeXPrt1.FileName := UniServerModule.NewCacheFileUrl(False, 'pdf', '', '', Result, True);
    PDFeXPrt1.DefaultPath := '';

    if not FileExists(ExtractFilePath(PDFeXPrt1.FileName)) then
      ForceDirectories(ExtractFilePath(PDFeXPrt1.FileName));   //创建目录

    Report1.PreviewOptions.AllowEdit := False;
    Report1.PrepareReport;
    Report1.Export(PDFeXPrt1);
  except on Exp : Exception do
    begin
      UniServerModule.Logger.AddLog(Exp.ToString);
    end;
  end;
end;
