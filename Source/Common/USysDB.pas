...
const
  ...
  sPopedom_ViewMYCusData  = 'I';                     //仅查看个人所属客户数据
  ...
  sTable_TransBase    = 'P_TransBase';               //短倒办卡
  sTable_TransBaseBak = 'P_TransBaseBak';            //短倒办卡
  sTable_Transfer     = 'P_Transfer';                //短倒明细单
  sTable_TransferBak  = 'P_TransferBak';             //短倒明细单
  sTable_TransferSW   = 'P_TransferSW';              //短倒销售明细单      声威业务
  sTable_OrderDtlTemp   = 'P_OrderDtlImportTemp';    //采购单导入临时表    声威业务
  ...
  sTable_CusCreditVif = 'Sys_CustomerCreditVerify';  //客户信用审批
  ...
  sSQL_NewCusCreditVerify = 'Create Table $Table(R_ID $Inc ,V_CreditID varChar(15),' +
       'V_PreFxMan varChar(32), V_Verify Char(1), V_VerMan varChar(32), ' +
       'V_VerDate DateTime, V_Memo varChar(160))';
  {-----------------------------------------------------------------------------
   信用审核:Sys_CustomerCreditVerify
   *.R_ID:编号
   *.V_CreditID:申请编号
   *.V_PreFxMan:申请人
   *.V_Verify:审核状态  Y/N/U   已批准 未通过  待审核
   *.V_VerMan:审核人
   *.V_VerDate:审核日期
   *.V_Memo:审核意见
  -----------------------------------------------------------------------------}
  ...
  sSQL_NewTransBase = 'Create Table $Table(R_ID $Inc, B_ID varChar(20),' +
       ...
       'B_DelMan varChar(32), B_DelDate DateTime, B_Memo varChar(500), '+
       'B_IsNei Char(1) Default ''N'', B_IsSale Char(1) Default ''N'', '+
       'B_KDValue decimal(15, 5) Not Null Default 0 ) ';
  {-----------------------------------------------------------------------------
   短倒基础表: TransBase
   *.R_ID: 编号
   *.B_ID: 短倒基础编号
  ...
  sSQL_NewTransfer = 'Create Table $Table(R_ID $Inc, T_ID varChar(20),' +
       ...
       'T_Status Char(1), T_NextStatus Char(1), T_Value $Float, T_Man varChar(32), T_Date DateTime,' +
       ...
       'T_LadeTime Datetime, T_LadeMan varChar(20))';
  {-----------------------------------------------------------------------------
   短倒明细表: Transfer
   *.R_ID: 编号
   *.T_ID: 短倒业务号
  ...
  sSQL_NewTransferSW = 'Create TABLE $Table(R_ID $Inc, T_ID varchar(20) , T_Card  varchar(32) , '+
       'T_Truck  varchar(15) , T_PID  varchar(15) , T_StockNo  varchar(32) , '+
       'T_StockName varchar(160) , T_PValue  decimal(15, 5) , T_PDate  datetime , '+
       'T_PMan  varchar(32) , T_MValue  decimal(15, 5) , T_MDate  datetime , '+
       'T_MMan  varchar(32) , T_Status  char(1) , T_NextStatus  char(1) , '+
       'T_Value  decimal(15, 5) , T_Man  varchar(32) , T_Date  datetime , '+
       'T_InTime  datetime , T_InMan  varchar(32) , T_OutFact  datetime , '+
       'T_OutMan  varchar(32) , T_LadeLine  varchar(15) , T_LineName  varchar(32) , '+
       'T_LadeTime Datetime , T_LadeMan  varchar(20)  Default ''N'', 	T_KDValue $Float Not Null Default 0, '+
       'T_CusName varchar(100) Not Null Default '''' )';
  {-----------------------------------------------------------------------------
   短倒销售放料明细表: TransferSW
   *.R_ID: 编号
   *.T_ID: 短倒业务号
   *.T_PID: 磅单编号
   *.T_Card: 磁卡号
  ...
  sSQL_NewAuxInfo = 'Create TABLE $Table(R_ID $Inc, T_ID varchar(20) ,A_PlaceId varchar(10) Default '''', '+
                    'A_PlaceName varchar(80) Default '''' )';
  {-----------------------------------------------------------------------------
   短倒地点: A_AuxInfo
   *.R_ID: 编号
   *.A_PlaceId: 短倒地点编号
  ...
  sSQL_NewZTTrucks = 'Create Table $Table(R_ID $Inc, T_Truck varChar(15),' +
       ...
       'T_InLadeFirst DateTime NOT NULL DEFAULT (''1900-01-01 00:00:00''))';
  {-----------------------------------------------------------------------------
   待装车队列: ZTTrucks
   *.R_ID: 记录号
   *.T_Truck: 车牌号
  ...
  sSQL_NewInvoiceWeek = 'Create Table $Table(W_ID $Inc, W_NO varChar(15),' +
       ...
       'W_CusName varChar(100), W_StockId Varchar(20), W_StockName varChar(100), '+
       'W_SaleManId Varchar(20), W_SaleMan Varchar(32))';
  {-----------------------------------------------------------------------------
   发票结算周期:InvoiceWeek
   *.W_ID:记录编号
   *.W_NO:周期编号
  ...
  sSQL_NewInvoiceReq = 'Create Table $Table(R_ID $Inc, R_Week varChar(15),' +
       ...
       'R_PreHasK $Float Default 0, R_ReqValue $Float, R_KPrice $Float, R_KYunFei $Float,' +
  ...
  sSQL_NewStockParam = 'Create Table $Table(P_ID varChar(15), P_Stock varChar(30),' +
       ...
       'P_3DYa varChar(20), P_28Ya varChar(20),	P_GanSuo varchar(20) NULL,' +
	     'P_NaiMo varchar(20) NULL, P_C4AF varchar(20) NULL, P_C3A varchar(20) NULL, ' +
	     'P_C3S varchar(20) NULL, P_7DZhe varchar(20) NULL, P_7DYa varchar(20) NULL, ' +
	     'P_3DShui varchar(20) NULL, P_7DShui varchar(20) NULL, P_28DShui varchar(20) NULL, '+
       'P_LvSuanSG varchar(20) NULL)';
  {-----------------------------------------------------------------------------
   品种参数:StockParam
   *.P_ID:记录编号
   *.P_Stock:品名
  ...
  sSQL_NewStockRecord = 'Create Table $Table(R_ID $Inc, R_SerialNo varChar(15),' +
       ...
       'R_Date DateTime, R_Man varChar(32), R_HHCValueBak varChar(20),' +
       'R_HHCValueHJ decimal(18, 2), R_GanSuo varchar(20) NULL, R_NaiMo varchar(20) NULL, R_C4AF varchar(20) NULL,' +
       'R_C3A varchar(20) NULL, R_C3S varchar(20) NULL, R_7DZhe1 varchar(20) NULL,' +
       'R_7DZhe2 varchar(20) NULL, R_7DZhe3 varchar(20) NULL, R_7DYa1 varchar(20) NULL,' +
       'R_7DYa2 varchar(20) NULL, R_7DYa3 varchar(20) NULL, R_7DYa4 varchar(20) NULL,' +
       'R_7DYa5 varchar(20) NULL, R_7DYa6 varchar(20) NULL, R_3DShui1 varchar(20) NULL,' +
       'R_3DShui2 varchar(20) NULL, R_7DShui1 varchar(20) NULL, R_7DShui2 varchar(20) NULL,' +
       'R_28DShui1 varchar(20) NULL, R_28DShui2 varchar(20) NULL, R_ZhuMoJi varchar(20) NULL,' +
       'R_ZhuMoJiValue varchar(20) NULL, R_LvSuanSG varchar(20) NULL)';
  {-----------------------------------------------------------------------------
   检验记录:StockRecord
   *.R_ID:记录编号
   *.R_SerialNo:水泥编号
...
begin
  ...
  AddSysTableItem(sTable_CusCreditVif, sSQL_NewCusCreditVerify);  // 多级审批
  ...
  AddSysTableItem(sTable_TransferSW, sSQL_NewTransferSW);
  AddSysTableItem(sTable_AuxInfo, sSQL_NewAuxInfo);
