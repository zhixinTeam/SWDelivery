...
//  参数[innHandle		已经打开的串口的句柄
//      [innAddr		机器的地址，有效取值（0到15）
//      [outnVersion		存机器的版本信息，读取成功会存储版本信息，
//  如 "TTCE_K720_V2.**"
//  返回值：	正确=0，错误=非0
...
//  参数[innHandle		已经打开的串口的句柄
//      [innAddr		机器的地址，有效取值（0到15）
//      [outnStatus		存储D1801状态信息，返回4个字节，详情请参K720照通讯协议
//      [outnRecord	存储该条命令的通讯记录
//
//  返回值：	正确=0，错误=非0
...
//  功能：	S70 卡寻卡
//  参数[innHandle	 已经打开的串口的句
//      [innAddr		机器的地址，有效取值（0到15）
//      [outnRecrod	存储该条命令的通讯记录
//  返回值： 正确=0，错误=非 0
...
//  功能：	S70 卡读取 ID 号
//  参数：[innHandle			已经打开的串口的句柄
//        [innAddr           机器地址，有限取值(0-15)
//        [outnCardID			存储读取的 卡片 ID 号
//        [outnRecord		存储该条命令的通讯记录
//  返回值： 正确=0，错误=非 0
...
//  功能S50 读取序列号
//  参数[inComHandle                 已经打开的串口的句柄
//      [inMacAddr           机器地址，有限取值(0-15)
//      [out_CardID		存储卡片序号 [outRecrodInfo	 存储该条命令的通讯记录
//  返回值： 正确=0，错误=非 0
...
//  参数[innHandle		已经打开的串口的句柄
//      [innAddr		机器的地址，有效取值（0到15）
//      [outnData		存储计数数据，前十个字节记录发卡数量最后一个字节记录回收卡数量
//      [outnRecord	存储该条命令的通讯记录
//
//  返回值：	正确=0，错误=非0
