...
procedure Create_SrvNC(out anInstance : IUnknown);
begin
  anInstance := TSrvNC.Create;
end;
...
begin
  ...
  begin
    ...
    FSrvNC := TROPooledClassFactory.Create('SrvNC',
                Create_SrvNC, TSrvNC_Invoker,
                FPoolSizeNC, TRoPoolBehavior(FPoolBehaviorNC));
  end;
end;
...
begin
  ...
  if Assigned(FSrvNC) then
  begin
    UnRegisterClassFactory(FSrvNC);
    FSrvNC := nil;
  end;
end;
