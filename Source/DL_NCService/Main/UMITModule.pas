  ...
  try
    ...
    begin
      ...
      begin
        ...
        if nStr = sFlag_NCFactory then
          gSysParam.FFactID := Fields[0].AsString;
        //fact id

        if nStr = sFlag_NCSrvRemote then
          gSysParam.FSrvRemote := Fields[0].AsString;
        //local mit

        if nStr = sFlag_NCServiceMIT then
...
begin
  ...
  gMessageScan.Stop;
  //停止消息扫描
  gTNCStatusChker.Resume;
  gTNCStatusChker.Terminate;
  {$IFDEF AutoChannel}
...
begin
  ...
  gTNCStatusChker:=TNCStatusChkThread.Create;
