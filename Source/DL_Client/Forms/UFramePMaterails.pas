...
procedure TfFrameMaterails.N8Click(Sender: TObject);
var nStr, nStockID: string;
begin
  if cxView1.DataController.GetSelectedCount > 0 then
  begin
    nStr := SQLQuery.FieldByName('M_Name').AsString;
    nStr := Format('确定要设置[ %s ]为短倒物料吗?', [nStr]);
    if not QueryDlg(nStr, sAsk) then Exit;

    nStockID := SQLQuery.FieldByName('M_ID').AsString;
    nStr := 'Update %s Set M_IsNei=''%s'' Where M_ID=''%s''';
    nStr := Format(nStr, [sTable_Materails, sFlag_Yes, nStockID]);
    FDM.ExecuteSQL(nStr);
...
procedure TfFrameMaterails.N9Click(Sender: TObject);
var nStr, nStockID: string;
begin
  if cxView1.DataController.GetSelectedCount > 0 then
  begin
    nStr := SQLQuery.FieldByName('M_Name').AsString;
    nStr := Format('确定要取消[ %s ]为短倒物料吗?', [nStr]);
    if not QueryDlg(nStr, sAsk) then Exit;
