...
//读取客户列表
function LoadNeiDaoStocks(const nList: TStrings; const nWhere: string = ''): Boolean;
...
//Parm: 交货单;车牌号;校验制卡开关
//Desc: 为nBill交货单制卡
function SetBillCard(const nBill,nTruck: string; nVerify: Boolean; nType: string): Boolean;
var nStr: string;
...
//  nStr := Format(nStr, [sTable_Bill, sTable_StockParam, nHID ]);   //
  ...
  if FDM.QueryTemp(nSR).RecordCount < 1 then
  begin
    nStr := '编号为[ %s ] 的提货记录已无效!!';
    nStr := Format(nStr, [nTHID]);
    ShowMsg(nStr, sHint); Exit;
  end;
